...
public class OwnAccountTransfer extends Activity implements OnClickListener
{
	...
	private void addBeneficiaries(String retval) 
	{
		...
		try {
			...
			ArrayAdapter<String> accs = new ArrayAdapter<String>(OwnAccountTransfer.this,R.layout.spinner_item, benfArr);
	...
	public void addBenfAccounts(String str)
	{
		try
		{
			ArrayList<String> arrList = new ArrayList<String>();
			...
			int noOfAccounts = allstr.length;
			arrList.add("Select Beneficiary");
			arrListTemp.add("Select Beneficiary");
			acArray = new Accounts[noOfAccounts];
			int j=0;
			for (int i = 0; i < noOfAccounts; i++)
			{
	...
	{ // logic to show input box
		...
		{
			Intent intet=new Intent(OwnAccountTransfer.this,FundTransferMenuActivity.class);
		...
		*/}
		...
		else if (v.getId() == R.id.sameBnkTranbtn_submit) 
		{
			...
			else if(strToAccNo.equalsIgnoreCase("Select Beneficiary"))
			{
				showAlert(getString(R.string.alert_098));
			}
			else if(creditAcc.equalsIgnoreCase(debitAcc))
			{
...
Log.e("debitAccno===same", debitAccno);
			...
			Intent in = new Intent(OwnAccountTransfer.this,TransferOTP.class);
			in.putExtra("var1", var1);
			   in.putExtra("var3", var3);
			   in.putExtras(bundle);
